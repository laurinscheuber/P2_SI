...
float Alphabeta(Player pl, Board b, unsigned depth, bool maximizingPlayer, float alpha, float beta, Move &best_move)
{
    ...
    {
        float ret = (maximizingPlayer) ? h1(pl, b) : h2(pl, b);
        if (DEB && (depth == chosen_depth))
            cout << "No movements from the initial depth. Returning " << ret << endl;
        return ret;
    }
    ...
    if (maximizingPlayer)
    {
        ...
        for (Sqlist::iterator it = sons.begin(); it != sons.end(); it++)
        {
            ...
            Move dummy_move;
            v = Alphabeta(Opponent(pl), child, depth - 1, false, alpha, beta, dummy_move);
    ...
    else
    {
        ...
        for (Sqlist::iterator it = sons.begin(); it != sons.end(); it++)
        {
            ...
            Move dummy_move;
            v = Alphabeta(Opponent(pl), child, depth - 1, true, alpha, beta, dummy_move);
